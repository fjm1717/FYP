...
output_path = '/home/spyros/Spyros/FYP/signature_ws/src/fyp/trajectory_output/elastic_constraint_output.csv'
...
#user force gains
kf = np.diag([0.1,0.12,0.15])
#elastic force gains
ke = np.diag([22.0,22.0,20.0])
...
with open(output_path, mode='w') as csv_file:
    ...
    while (xbox[3]==0):
        ...
        #upload state to csv
        writer.writerow({'x': str(robot.x), 'y': str(robot.y), 'z': str(robot.z), 'fx': str(elastic_force[0]), 'fy': str(elastic_force[1]), 'fz': str(elastic_force[2]), 'time': str(time)})
